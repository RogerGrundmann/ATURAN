...
*
*/
void cUranusModel::UranusCellStructure(){
...
//    init_v_or_w(w, 90, 5.0, -5.0); //lat: 0
    init_v_or_w(w, 90, -0.65, -0.65); //lat: 0
    
    
    
//polar cells w
    //northern polar cell
    init_v_or_w(w, 0, 0.0, 0.0); //lat: 90

    //southern polar cell
    init_v_or_w(w, 180, 0.0, 0.0); //lat: -90



//Ferrel cells w
    //northern Ferrel cell
    init_v_or_w(w, 30, 0.6, 0.0); //lat: 60

    //southern Ferrel cell
    init_v_or_w(w, 150, 0.6, 0.0); //lat: -60
 
 
// Hadley cells w
    //northern Hadley cell
    init_v_or_w(w, 60, 0.6, 0.0); //lat: 30
    //southern Hadley cell
    init_v_or_w(w, 120, 0.6, 0.0); //lat: -30



    //northern Hadley cell max velocity at 45
//    init_v_or_w(w, 45, 50.0, 12.5); //lat: 45
    init_v_or_w(w, 45, 1.85, 0.0); //lat: 45

    //southern Hadley cell max velocity at 135
//    init_v_or_w(w, 135, 50.0, 12.5); //lat: 135
    init_v_or_w(w, 135, 1.85, 0.0); //lat: 135
...
*/
/*
void cUranusModel::smooth_transition(Array &u, Array &v, Array &w, int lat){
...
*
*/
void cUranusModel::form_diagonals(Array &a, int start, int end){
...
*
*/
void  cUranusModel::init_u(Array &u, int j){
//    cout << endl << "      AGCM: init_u" << endl;
    float ua_00 = 0.2894,  // omega = vertical pressure velocity: u = - omega/(density*g) in m/s, omega in mb/d
          ua_30 = 0.2315,  // u = - 1.1574e⁻³ * omega
          ua_60 = 0.1736,  // Understanding the Effect of Convective Momentum Transport on Climate Simulations: The Role of Convective Heating
//          ua_90 = 0.11574;  // authors: Xiaoliang Song, Xiaoliang Wu, Guang Zhang and Raymond W. Arritt, Journal of Climate vol 21 pp. 5034, Oct. 2008 and ResearchGate
          ua_90 = 0.01;  // authors: Xiaoliang Song, Xiaoliang Wu, Guang Zhang and Raymond W. Arritt, Journal of Climate vol 21 pp. 5034, Oct. 2008 and ResearchGate
    ...
    for(int k = 0; k < km; k++){
        for(int i = 0; i < tropopause_layer; i++){
            ...
            if(layer_height < half_tropopause_height){    
                ratio = layer_height/(half_tropopause_height/2.2);  // factor is sensitive for cross section, 3.0 too steep 
                switch(j){
...
*
*/
void  cUranusModel::init_v_or_w(Array &v_or_w, int j, 
...
*
*/
void  cUranusModel::init_v_or_w_above_tropopause(Array &v_or_w, int j, 
    double coeff){
